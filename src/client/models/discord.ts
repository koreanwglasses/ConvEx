...
export const fetchMessages = async ({
  ...
  while (true) {
    ...
    if (lastMessage?.createdTimestamp <= creationTimeAfter) break;
  ...
  while (true) {
    ...
    if (firstMessage?.createdTimestamp >= creationTimeBefore) break;
  ...
  return messages.filter(
    (message) =>
      creationTimeAfter <= message.createdTimestamp &&
      message.createdTimestamp <= creationTimeBefore
  );
};
